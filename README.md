<!--
  HOW TO USE THIS FILE
  1. Create a new repository named EXACTLY your GitHub username (e.g. github.com/stas/stas).
  2. Make it public and add this file as README.md.
  3. Replace every {{PLACEHOLDER}} below with your real info, then delete this comment block.
-->

<h1 align="center">Hi, I'm Stas 👋</h1>
<p align="center">
  <b>UAV Designer × Software Developer</b> — I build automation, AI, and internal tools that turn messy operational problems into reliable software.
</p>

<p align="center">
  📍 Warsaw, Poland &nbsp;·&nbsp; 🛠️ Manufacturing automation &nbsp;·&nbsp; 🤖 Practical AI &nbsp;·&nbsp; 🚁 Drone design
</p>

---

### About me

- 🔧 I work where **hardware meets software** — by day I design UAVs and develop the internal systems that keep a manufacturing operation running.
- ⚙️ I build **business automation** with Python and Odoo: data-entry automation, inventory/label tracking, and quality-control systems.
- 🧠 I bring **AI into real workflows** — computer vision, real-time pose estimation, and LLM-powered automation, not just demos.
- 🎮 For fun, I build things that move: a motion-tracking rhythm game (Python + MediaPipe → Unity), KSP mods, and the occasional 3D-printed contraption.
- 💬 Open to freelance work — say hi on [Upwork]({{UPWORK_PROFILE_URL}}).

---

### Tech I work with

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C#](https://img.shields.io/badge/C%23-512BD4?style=flat-square&logo=csharp&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)

**Automation & Backend**

![Odoo](https://img.shields.io/badge/Odoo-714B67?style=flat-square&logo=odoo&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST%20APIs-005571?style=flat-square&logo=fastapi&logoColor=white)

**AI / ML**

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)
![MCP](https://img.shields.io/badge/LLMs%20%26%20MCP-1E6A4D?style=flat-square&logo=anthropic&logoColor=white)

**Engineering & Tools**

![Unity](https://img.shields.io/badge/Unity-000000?style=flat-square&logo=unity&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![SolidWorks](https://img.shields.io/badge/SolidWorks-E2231A?style=flat-square&logo=dassaultsystemes&logoColor=white)

---

### Featured projects

> 🔒 A lot of my automation work lives in private/company repos, so here's what I can show. Replace the links with your public repos.

| Project | What it is | Stack |
|---|---|---|
| 🎵 **Motion Rhythm Game** | A Kinect-style rhythm game — Python streams body-pose landmarks over UDP to a Unity game that handles note-spawning, scoring, and a live skeleton visualizer. | `Python` · `MediaPipe` · `Unity` · `C#` |
| ⚙️ **Business Automation Tooling** | Internal tools for a manufacturing company: automated data entry, label/inventory tracking, and a quality-control system. | `Python` · `Odoo` · `PostgreSQL` |

---

### GitHub stats

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username={{GITHUB_USERNAME}}&show_icons=true&hide_border=true&bg_color=00000000&title_color=1E6A4D&icon_color=3E9A77&text_color=808080" alt="GitHub stats" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username={{GITHUB_USERNAME}}&layout=compact&hide_border=true&bg_color=00000000&title_color=1E6A4D&text_color=808080" alt="Top languages" />
</p>

---

### Let's connect

[![Upwork](https://img.shields.io/badge/Upwork-6FDA44?style=flat-square&logo=upwork&logoColor=white)]({{[UPWORK_PROFILE_URL](https://www.upwork.com/freelancers/~01335cb44a9acc19b4)}})
[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:{{sakstas6b@gmail.com}})

<p align="center"><i>🌍 Working in English · Polish · Russian</i></p>
